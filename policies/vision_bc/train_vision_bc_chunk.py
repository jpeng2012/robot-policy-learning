...
class VisionChunkBCModel(nn.Module):
    def __init__(
            self,
        ...
        self.motion_head = nn.Sequential(
            ...
            nn.Linear(256, horizon * (action_dim-1)),
            nn.Tanh(),
        )
    ...
    def forward(self, agent, wrist, proprio):
        ...
        motion = motion.reshape(B, -1, 6)
        gripper = self.gripper_head(x)
        gripper = gripper.reshape(B, -1, 1)
...
save_path = "policies/vision_bc/vision_bc_level3_hist_1_4_chunk.pth"
...
optimizer = torch.optim.AdamW([
    ...
    {
        "params": policy.motion_head.parameters(),
        "lr": 1e-3,
    },
    {
        "params": policy.gripper_head.parameters(),
        "lr": 1e-3,
    }
])
...
for epoch in range(num_epochs):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        pred_motion = pred_action[..., :6]
        pred_gripper = pred_action[..., 6]

        target_motion = action[..., :6]
        target_gripper = (
            action[..., 6] > 0
        ).float()

        motion_loss = ((pred_motion - target_motion) ** 2).mean(dim=(1, 2))
